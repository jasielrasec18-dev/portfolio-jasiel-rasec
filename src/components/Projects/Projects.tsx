  ...
  return (
    <article className="group overflow-hidden rounded-[24px] border border-white/10 bg-[rgba(10,10,20,0.95)] shadow-[0_24px_80px_rgba(0,0,0,0.35)] transition-all duration-300 hover:-translate-y-2 hover:border-neonMagenta/50 hover:shadow-[0_30px_100px_rgba(204,102,255,0.2)]">
      <div className="aspect-[16/10] w-full overflow-hidden">
        <img src={project.image} alt={project.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-400 group-hover:scale-104" />
      </div>
      ...
      <div className="p-6 sm:p-7">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {project.technologies.map((tech) => (
            <span key={tech} className="rounded-full border border-neonPurple/40 bg-neonPurple/10 px-3 py-1 text-sm text-white">
              {tech}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          {project.liveUrl && (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-gradient-to-r from-[#b84dff] to-[#6f00ff] px-5 py-2.5 text-sm font-semibold text-white transition-transform duration-300 hover:-translate-y-0.5 hover:opacity-90">
              Ver Projeto
            </a>
          )}
          {project.githubUrl && (
...
function Projects() {
  return (
    <section className="bg-black px-4 py-24 text-white sm:px-6 lg:px-8" id="projetos">
